...
export function Tile({
	...
	return (
		<Link
			{...props}
			className={cn(
				"relative overflow-hidden uppercase group flex tracking-widest text-lg md:text-xl shadow-lg",
				className
			)}>
			{image && (
				<StorageImage
					...
					image={image}
					priority={priority}
					className=" w-full h-full group-hover:scale-110 transition-all brightness-50 object-cover"
				/>
			)}
			<h2 className="p-8 group-hover:scale-110 transition-all text-center pointer-events-none w-full flex items-center justify-center font-semibold absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
